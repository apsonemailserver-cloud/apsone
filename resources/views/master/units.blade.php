...
<div class="container-xxl flex-grow-1 container-p-y">
    <div class="py-4">
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-1 mb-4">
            <div>
                <h4 class="fw-bold mb-1">Master Units</h4>
                <p class="text-muted mb-0" style="font-size:0.875rem;">Kelola data master unit organisasi.</p>
            </div>
                ...
                <div class="table-responsive">
                    <table class="table align-middle">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex gap-2">
                                        <button type="button" class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#editUnitModal{{ $item->id }}">
                                            <i class="ti ti-pencil me-1"></i> Edit
                                        </button>
                                        <form action="{{ route('master.units.destroy', $item->id) }}" method="POST" class="d-inline" id="deleteForm{{ $item->id }}">
                                            @csrf
                                            @method('DELETE')
                            ...
                            <div class="modal fade" id="editUnitModal{{ $item->id }}" tabindex="-1" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content">
                                        <form action="{{ route('master.units.update', $item->id) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                            ...
                            @empty
                            <tr>
                                <td colspan="4" class="text-center text-muted py-4">Belum ada data unit.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form action="{{ route('master.units.store') }}" method="POST">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title fw-bold">Tambah Unit Baru</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label">Nama Unit <span class="text-danger">*</span></label>
                        <input type="text" name="name" class="form-control" placeholder="Contoh: HEAD OFFICE" required>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
...
<script>
    function confirmDelete(id, name) {
        Swal.fire({
            title: 'Hapus Unit?',
            text: `Apakah Anda yakin ingin menghapus unit "${name}"?`,
            icon: 'warning',
