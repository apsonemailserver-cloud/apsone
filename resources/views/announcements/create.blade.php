...
<div class="container-xxl flex-grow-1 container-p-y">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-2 mb-4">
        <div>
            ...
            <p class="text-muted mb-0">Terbitkan pengumuman baru.</p>
        </div>
        ...
        <div class="card-body">
            ...
            <form action="{{ route('announcements.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="title" class="form-control form-control-lg" placeholder="Contoh: Informasi Perpanjangan Kontrak Kerja & PAS Bandara" value="{{ old('title') }}" required>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <div class="dropdown position-relative" id="stationMultiSelectDropdown">
                        <button class="btn btn-outline-secondary w-100 text-start d-flex align-items-center justify-content-between py-2 px-3 bg-white" type="button" data-bs-toggle="dropdown" data-bs-auto-close="outside" aria-expanded="false" style="border-radius: 0.375rem; min-height: 44px;">
                            <div class="d-flex flex-wrap gap-1 align-items-center" id="createSelectedStationsBadge">
                        ...
                        <div class="dropdown-menu w-100 p-2 shadow-lg border-0" style="max-height: 260px; overflow-y: auto; border-radius: 0.375rem;">
                            <div class="form-check p-2 rounded hover-bg-light cursor-pointer">
                                <input class="form-check-input ms-0 me-2" type="checkbox" name="target_stations[]" value="ALL" id="createStationAll" {{ empty(old('target_stations')) || in_array('ALL', old('target_stations', []), true) ? 'checked' : '' }}>
                                <label class="form-check-label fw-bold text-dark cursor-pointer" for="createStationAll">
                                    Semua Station (ALL)
                                </label>
                            </div>
                            ...
                            @foreach($stations as $st)
                                <div class="form-check p-2 rounded hover-bg-light cursor-pointer">
                                    <input class="form-check-input station-specific-checkbox ms-0 me-2" type="checkbox" name="target_stations[]" value="{{ $st->code }}" id="createStation_{{ $st->code }}" {{ in_array($st->code, old('target_stations', []), true) ? 'checked' : '' }}>
                                    <label class="form-check-label text-dark cursor-pointer" for="createStation_{{ $st->code }}">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="content" class="form-control" rows="8" placeholder="Tuliskan instruksi atau pengumuman lengkap untuk staf di sini..." required>{{ old('content') }}</textarea>
                </div>
                ...
                <div class="d-flex align-items-center justify-content-end gap-2">
                    ...
                    <button type="submit" class="btn btn-primary">
                        <i class="ti ti-send me-1"></i>Terbitkan Pengumuman
                    </button>
                </div>
            </form>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const allCheckbox = document.getElementById('createStationAll');
        const specificCheckboxes = document.querySelectorAll('.station-specific-checkbox');
        const badgeContainer = document.getElementById('createSelectedStationsBadge');
